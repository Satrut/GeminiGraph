...
parser.add_argument('-e', type=int, default=-1, help='number of edges in the graph, randomized if not given')
parser.add_argument('--undirected', action='store_true', help='generate an undirected graph instead of the default directed one')
parser.add_argument('-o', required=True, type=str, help='name of the generated file')

opt = parser.parse_args()

max_edges = opt.v * (opt.v - 1) if not opt.undirected else opt.v * (opt.v - 1) // 2
...
selection = sorted(np.random.choice(len(edges_list), opt.e))
selected_edges = edges_list[selection]

# os.makedirs(os.path.dirname(opt.o), exist_ok=True)

with open(opt.o, 'wt') as f:
    # f.write(f'{"an un" if opt.undirected else "a "}directed graph with ' \
    #   f'{opt.v} {"vertices" if opt.v > 1 else "vertex"} and {opt.e} {"edges" if opt.e > 1 else "edge"}\n')
